...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const category = await anthropicCall(
      'Extract the product/brand category from an audit verdict. Return 1–3 words only, no punctuation, no preamble. Examples: "natural supplements", "running shoes", "skincare".',
      `Verdict: "${verdict}"`,
      anthropicKey,
      20,
    )
